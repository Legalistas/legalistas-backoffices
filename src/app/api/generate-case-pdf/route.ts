...
export async function POST(request: NextRequest) {
  try {
    ...
    doc.text(`Generado el ${formatDate(new Date())}`, pageWidth - 20, logoY + 4, { align: "right" });
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(9, 164, 181);
    doc.text(`Caso #${data.number || data.id}`, pageWidth - 20, logoY + 10, { align: "right" });
    ...
    const serviceLabel = data.servicesId ? serviceLabels[data.servicesId] || "-" : "-";
    const rawResult = data.status && ["IN_PROGRESS", "WON", "LOST"].includes(data.status) ? data.status : null;
    const resultLabel = rawResult ? resultLabels[rawResult] || "En Progreso" : "En Progreso";
    ...
    doc.text(stageLabel, pageWidth - 20 - resultWidth - 8, yPos, { align: "right" });

    // Servicio badge
    doc.setTextColor(13, 148, 136);
    doc.text(serviceLabel, pageWidth - 20 - resultWidth - 8 - stageWidth - 8, yPos, { align: "right" });
    ...
    for (let i = 1; i <= totalSteps; i++) {
      ...
      doc.text(stageLabels[i] || "", cx, yPos + circleRadius + 4, { align: "center" });
    }
    ...
    autoTable(doc, {
      ...
      theme: "plain",
      styles: { fontSize: 10, cellPadding: 3, lineColor: [229, 231, 235], lineWidth: 0.1 },
      columnStyles: {
    ...
    if (data.responsibleLawyer?.name) personsData.push(["Abog. Responsable", data.responsibleLawyer.name]);
    if (data.internalLawyer?.name) personsData.push(["Abog. Interno", data.internalLawyer.name]);
