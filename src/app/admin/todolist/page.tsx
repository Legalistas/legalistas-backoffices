...
export default function TodoListPage() {
	...
	const loadStats = async () => {
		...
		try {
			const statsData = await todolistService.getStats(
				session.user.accessToken,
			);
			setStats(statsData);
		} catch (error) {
	...
	const handleDelete = async (id: number) => {
		if (!session?.user?.accessToken) return;
		if (!(await confirm({ description: "¿Estás seguro de eliminar esta tarea?", confirmLabel: "Eliminar" }))) return;
	...
	const handleUpdateStatus = async (id: number, status: TodoStatus) => {
		...
		try {
			await todolistService.updateTodoList(session.user.accessToken, id, {
				status,
			});
			...
			<div className="flex gap-4 mb-6">
				<div className="flex items-center gap-2">
					...
					<select
						...
						className="h-11 w-full rounded-lg border bg-transparent text-gray-800 border-gray-300 px-4 py-2.5 text-sm shadow-theme-xs focus:outline-hidden focus:ring-3 focus:border-primary/30 focus:ring-primary/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-primary/40"
					>
