...
import { MessageSquare, FolderOpen, Clock, Mail, Eye, Ban, RefreshCw, CheckCircle, Terminal } from "lucide-react"
...
export default function ConsultationsPage({ searchParams }: ConsultationsPageProps) {
    ...
    const fetchConsultations = useCallback(async () => {
        try {
            ...
            const url = new URL(CASES_CONSULTATIONS_GET_ALL_ENDPOINT, window.location.origin)
        ...
        <div>
            <div className="flex flex-col gap-6 mb-2">
                <div className="flex items-center justify-between">
                    <h1 className="text-3xl font-bold tracking-tight">Gestor de Consultas</h1>
                    <div className="flex items-center gap-2">
                        <Button onClick={fetchConsultations} disabled={loading} variant="outline" size="sm">
                                            ...
                                            <TableRow key={consultation.id} className="hover:bg-gray-50 transition-colors ">
                                                <TableCell className="px-6 py-4 font-medium">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="line-clamp-1">{consultation.title}</span>
                                                    </div>
                                                </TableCell>
                                                <TableCell className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        <FolderOpen className="h-4 w-4 text-muted-foreground" />
                                                        <span className="line-clamp-1">{consultation.cases?.title || "N/A"}</span>
                                                    </div>
                                                </TableCell>
                                                <TableCell className="px-6 py-4 text-muted-foreground">
                                                    <p className="line-clamp-2">{consultation.lastMessagePreview || "No hay mensajes"}</p>
                                                </TableCell>
                                                <TableCell className="px-6 py-4">
                                                    <Badge variant="light" color={getStatusBadgeVariant(consultation.status)} className="capitalize">
                                                ...
                                                <TableCell className="px-6 py-4 text-muted-foreground">
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        {new Date(consultation.createdAt).toLocaleDateString("es-ES", dateFormatOptions)}
                                                    </div>
                                                </TableCell>
                                                ...
                                                <TableCell className="px-6 py-4 text-right">
                                                    <Link href={`/admin/consultations/${consultation.id}`}>
                                                        <Button variant="ghost" size="icon" aria-label={`Ver consulta ${consultation.title}`}>
