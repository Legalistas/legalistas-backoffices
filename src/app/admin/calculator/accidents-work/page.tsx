...
import { Calculator, File, Search, X, Download, Save, User, FileText, Calendar, Cake, PercentCircle, TrendingUp, BarChart3, Copy } from "lucide-react";
import { useState, useEffect, useRef, useCallback } from "react";
import { useSession } from "next-auth/react";
import { useSearchParams } from "next/navigation";
import { CALCULATOR_CAUSES_LIST_ENDPOINT, API_BASE_URL } from "@/constant/api-endpoints";
...
export default function AccidentsWorkPage() {
    ...
    const [remuneraciones, setRemuneraciones] = useState<{ periodo: string; haberes: string; ripteDelMes: number | null; haber_ajustado: number | null }[]>([]);
    const [porcentajesRipte, setPorcentajesRipte] = useState<{ mesPeriodo: string; mesCorriendo: string; riptePercentage: number | null }[]>([]);
    ...
    }, [selectedFile?.id, accidentDate, customerAge, disabilityPercentage, dateUntil, 
        selectedRipte, selectedRipteHasta, remuneraciones, porcentajesRipte, 
        activar20Porciento, pisoMinimo, activarPisoMinimo, saveToLocalStorage]);
    ...
    useEffect(() => {
        ...
        if (savedData) {
            ...
            if (savedData.disabilityPercentage !== null) setDisabilityPercentage(savedData.disabilityPercentage);
            ...
            if (savedData.selectedRipteHasta) setSelectedRipteHasta(savedData.selectedRipteHasta);
            ...
            if (savedData.porcentajesRipte) setPorcentajesRipte(savedData.porcentajesRipte);
            if (savedData.activar20Porciento !== undefined) setActivar20Porciento(savedData.activar20Porciento);
            if (savedData.pisoMinimo !== null) setPisoMinimo(savedData.pisoMinimo);
            if (savedData.activarPisoMinimo !== undefined) setActivarPisoMinimo(savedData.activarPisoMinimo);
        }
    ...
    useEffect(() => {
        const fetchRiptes = async () => {
            if (!accidentDate || !session?.user?.accessToken) {
                console.log('No se puede cargar RIPTEs:', { accidentDate, hasToken: !!session?.user?.accessToken });
                setRiptes([]);
                return;
            }
            ...
            try {
                ...
                console.log('Para fecha:', { accidentDate, year, month: monthCapitalized });
                ...
                const ripteMonth = accidentDateObj.toLocaleString('es-ES', { month: 'long' });
                const ripteMonthCapitalized = ripteMonth.charAt(0).toUpperCase() + ripteMonth.slice(1);

                const ripteVigente = data.data?.find(
                    (r: any) => r.month === ripteMonthCapitalized && r.year === ripteYear
                );
                console.log('RIPTE Vigente (1 año atrás) encontrado:', ripteVigente, 'Mes:', ripteMonthCapitalized, 'Año:', ripteYear);
                if (ripteVigente) {
    ...
    const isValidFile = (file: CaseFile, customer: { userProfile: { birthDate: string | null } | null }) => {
    ...
    const filteredCauses = causes.filter((cause) => {
        // Filtro de búsqueda
        const matchesSearch = cause.customer.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            cause.files.some(file => file.cuij?.toLowerCase().includes(searchTerm.toLowerCase()));

        if (!matchesSearch) return false;

        // Debe tener al menos un expediente válido
        const hasValidFiles = cause.files && cause.files.length > 0 &&
    ...
    const handleRemoveSelection = () => {
        ...
        if (selectedFile?.id) {
            try {
                ...
                console.log('Datos limpiados del localStorage para expediente:', selectedFile.id);
            } catch (error) {
    ...
    useEffect(() => {
        ...
        const hayHaberesExistentes = remuneraciones.some(row => row.haberes && row.haberes !== '');
        if (hayHaberesExistentes && remuneraciones.length > 0) {
            console.log('Tabla ya tiene haberes cargados, no regenerando para preservar datos');
            return;
        }

        console.log('Generando tabla basada en RIPTE Vigente:', selectedRipte.month, selectedRipte.year);
        ...
        for (let i = 0; i < 12; i++) {
            ...
            const ripteDelMes = allRiptes.find(r => r.month === monthName && r.year === year);
        ...
        console.log('Nueva tabla generada con', rows.length, 'filas. Primera:', rows[0]?.periodo, 'Última:', rows[rows.length - 1]?.periodo);
    ...
    useEffect(() => {
        ...
        console.log('Generando tabla de porcentajes RIPTE desde:', selectedRipte.month, selectedRipte.year, 'hasta:', selectedRipteHasta.month, selectedRipteHasta.year);
        ...
        const fechaDesde = new Date(selectedRipte.year, monthNames.indexOf(selectedRipte.month), 1);
        const fechaHasta = new Date(selectedRipteHasta.year, monthNames.indexOf(selectedRipteHasta.month), 1);
        ...
        while (tempDate <= fechaHasta) {
            ...
            const ripteActual = allRiptes.find(r => r.month === monthName && r.year === year);
            ...
            const ripte3MesesAtras = allRiptes.find(r => r.month === month3MA && r.year === year3MA);
        ...
        console.log('Tabla de porcentajes generada con', rows.length, 'filas desde', selectedRipte.month, selectedRipte.year, 'hasta', selectedRipteHasta.month, selectedRipteHasta.year);
    ...
    useEffect(() => {
        ...
        const hayHaberes = remuneraciones.some(row => row.haberes && row.haberes !== '');
        ...
        const yaCalculado = remuneraciones.some(row => {
            if (row.haberes && row.haber_ajustado && row.ripteDelMes) {
                const expectedValue = (Number(selectedRipte.value) / row.ripteDelMes) * parseFloat(row.haberes);
        ...
        console.log('Recalculando haberes ajustados con RIPTE:', selectedRipte.value);
    ...
    const handleHaberesChange = (index: number, value: string) => {
        ...
        if (cleanValue && !isNaN(parseFloat(cleanValue))) {
            newRemuneraciones[index].haber_ajustado = calcularHaberAjustado(index, cleanValue);
        } else {
    ...
    const hayHaberesIngresados = remuneraciones.some(row => row.haberes && row.haberes !== '');
    ...
    const promedioHaberesAjustados = remuneraciones.length > 0 ? totalHaberesAjustados / remuneraciones.length : 0;
    ...
    const ibmTotal = (promedioHaberesAjustados * (tasaDeVariacion / 100)) + promedioHaberesAjustados;

    // Calcular coeficiente de edad: (65 / edad del autor)
    const coeficienteEdad = customerAge !== null && customerAge > 0 ? (65 / customerAge) : 0;

    // Calcular TOTAL FÓRMULA: SUMA(53*IBM TOTAL*PORCENTAJE INCAPACIDAD*COEFICIENTE EDAD)/100
    const totalFormula = (53 * ibmTotal * (disabilityPercentage || 0) * coeficienteEdad) / 100;
    ...
    const totalPisoMinimo = (pisoMinimo && disabilityPercentage) ? (pisoMinimo * disabilityPercentage / 100) : 0;
    ...
    const calcularIntereses = () => {
        ...
        for (let año = añoAccidente; año <= añoActual; año++) {
            ...
            if (año === añoAccidente && año === añoActual) {
                ...
                diasDelAño = Math.ceil((hoy.getTime() - fechaAccidente.getTime()) / (1000 * 60 * 60 * 24));
            } else if (año === añoAccidente) {
                // Primer año: desde accidente hasta fin del año
                const finDelAño = new Date(año, 11, 31, 23, 59, 59);
                diasDelAño = Math.ceil((finDelAño.getTime() - fechaAccidente.getTime()) / (1000 * 60 * 60 * 24)) + 1;
            } else if (año === añoActual) {
                // Año actual: desde inicio del año hasta hoy
                const inicioDelAño = new Date(año, 0, 1);
                diasDelAño = Math.ceil((hoy.getTime() - inicioDelAño.getTime()) / (1000 * 60 * 60 * 24)) + 1;
            } else {
    ...
    const handleSaveLiquidacion = async () => {
        if (!selectedCause || !selectedFile || !session?.user?.accessToken) {
            alert('Por favor seleccione una causa y expediente primero');
            return;
        }
        ...
        console.log('Haberes ingresados:', remuneraciones.filter(r => r.haberes && r.haberes !== ''));
        
        if (remuneraciones.length === 0) {
            alert('⚠️ No hay datos de remuneraciones para guardar. Asegúrese de haber ingresado la información necesaria.');
            return;
        }
        ...
        try {
            ...
            remuneraciones.forEach((rem, index) => {
                console.log(`  [${index}] ${rem.periodo}: haberes="${rem.haberes}", ripte=${rem.ripteDelMes}, ajustado=${rem.haber_ajustado}`);
            });
            ...
            console.log('Solo calculationData:', JSON.stringify(calculationData, null, 2));
            ...
            alert('✅ Liquidación guardada exitosamente en el expediente');

            // Verificar inmediatamente lo que se guardó
            if (result.liquidation && result.liquidation.calculationData) {
                console.log('Datos guardados en el servidor:', result.liquidation.calculationData);
                console.log('Remuneraciones en servidor:', result.liquidation.calculationData.remuneraciones);
            }
    ...
    const fetchSavedLiquidations = async (caseId: number) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/lrt/liquidations/${caseId}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${session.user.accessToken}`,
    ...
    const handleLoadLiquidation = async (liquidationId: number) => {
        ...
        try {
            ...
            if (data.remuneraciones && Array.isArray(data.remuneraciones)) {
                console.log('Estableciendo remuneraciones ANTES que selectedRipte:', data.remuneraciones);
            ...
            setTimeout(() => {
                ...
                if (data.edad !== null && data.edad !== undefined) setCustomerAge(data.edad);
                if (data.incapacidad !== null && data.incapacidad !== undefined) setDisabilityPercentage(data.incapacidad);
                ...
                if (data.activar20Porciento !== undefined) setActivar20Porciento(data.activar20Porciento);
                if (data.pisoMinimo !== null && data.pisoMinimo !== undefined) setPisoMinimo(data.pisoMinimo);
                if (data.activarPisoMinimo !== undefined) setActivarPisoMinimo(data.activarPisoMinimo);
            ...
            alert('✅ Liquidación cargada exitosamente');
        } catch (error) {
        ...
        <div className="container mx-auto p-4">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-4">
                ...
                <div className="relative" ref={searchRef}>
                    ...
                    {showSuggestions && filteredCauses.length > 0 && (
                        <div className="absolute z-10 w-full mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg max-h-80 overflow-y-auto">
                            {filteredCauses.map((cause) => (
                                <div key={cause.id} className="border-b border-gray-100 dark:border-gray-700 last:border-0">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-sm text-gray-600 dark:text-gray-400 flex flex-wrap gap-3">
                                                    ...
                                                    {cause.customer.userProfile?.birthDate && (
                                                        <span className="flex items-center gap-1">
                                                            <Cake size={12} /> {calculateAge(cause.customer.userProfile.birthDate)} años
                                                        </span>
                                                    )}
                                                    {cause.files.length > 0 && cause.files[0].disabilityPercentage !== null && (
                                                        <span className="flex items-center gap-1">
                                                            <PercentCircle size={12} /> {cause.files[0].disabilityPercentage}%
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
                                                ...
                                                >
                                                    <div className="flex justify-between items-center">
                                                        <span className="font-medium text-gray-900 dark:text-white flex items-center gap-1.5">
                                                            ...
                                                            {file.cuij || `Expediente #${file.id}`} - {cause.customer.name} {file.filesParts && file.filesParts[0]?.name ? `C/${file.filesParts[0].name}` : ""} S/ {getProcessTypeLabel(file.typeProcessId)}.
                                                        </span>
                    ...
                    {showSuggestions && searchTerm && filteredCauses.length === 0 && !loading && (
                        <div className="absolute z-10 w-full mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-6 text-center">
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">No se encontraron resultados para <span className="font-medium">{searchTerm}</span></p>
                        </div>
                    )}
                </div>
            </div>
                            ...
                            <div className="space-y-2.5">
                                <div>
                                    <div className="flex items-center gap-1 mb-0.5">
                                        <User className="text-gray-500 dark:text-gray-400" size={12} />
                                        <label className="text-[16px] font-medium text-gray-600 dark:text-gray-400">
                                            Cliente
                                        </label>
                                    </div>
                                    <p className="text-sm font-semibold text-gray-900 dark:text-white ml-3.5">{selectedCause.customer.name}</p>
                                </div>

                                <div>
                                    <div className="flex items-center gap-1 mb-0.5">
                                        <FileText className="text-gray-500 dark:text-gray-400" size={12} />
                                        <label className="text-[16px] font-medium text-gray-600 dark:text-gray-400">
                                            Expediente
                                        </label>
                                    </div>
                                    <p className="text-sm font-semibold text-gray-900 dark:text-white ml-3.5">{selectedFile.cuij || 'Sin CUIJ'}</p>
                                </div>

                                <div>
                                    <div className="flex items-center gap-1 mb-0.5">
                                        <Calendar className="text-gray-500 dark:text-gray-400" size={12} />
                                        <label className="text-[16px] font-medium text-gray-600 dark:text-gray-400">
                                            Fecha Accidente
                                        </label>
                                    </div>
                                ...
                                <div>
                                    <div className="flex items-center gap-1 mb-0.5">
                                        <Cake className="text-gray-500 dark:text-gray-400" size={12} />
                                        <label className="text-[16px] font-medium text-gray-600 dark:text-gray-400">
                                            Edad
                                        </label>
                                    </div>
                                    <p className="text-sm font-semibold text-gray-900 dark:text-white ml-3.5">
                                        {customerAge !== null ? `${customerAge} años` : 'No registrada'}
                                    </p>
                                </div>

                                <div>
                                    <div className="flex items-center gap-1 mb-0.5">
                                        <PercentCircle className="text-gray-500 dark:text-gray-400" size={12} />
                                        <label className="text-[16px] font-medium text-gray-600 dark:text-gray-400">
                                            Incapacidad
                                        </label>
                                    </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-3">
                            <div className="flex items-center justify-between mb-3 pb-2 border-b border-gray-200 dark:border-gray-700">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
                                        Cálculo LRT - {selectedFile.cuij || `Exp. #${selectedFile.id}`}
                                    </h2>
                                </div>
                                <button
                            ...
                            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 mb-3">
                                <div className="flex items-center gap-1.5 mb-2">
                                    <BarChart3 className="text-gray-600 dark:text-gray-400" size={14} />
                                    <h3 className="text-sm font-semibold text-gray-900 dark:text-white">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <div>
                                        ...
                                        <input
                                            type="number"
                                            className={`w-full px-2 py-1.5 text-sm border rounded-md text-gray-900 dark:text-white focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all ${hasCustomerBirthDate
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="date"
                                            className="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all"
                                            value={dateUntil || new Date().toLocaleDateString('en-CA')}
                            ...
                            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 mb-3">
                                <div className="grid grid-cols-2 gap-2">
                                    <div>
                                        <div className="flex items-center gap-1 mb-1">
                                            <TrendingUp className="text-gray-500 dark:text-gray-400" size={12} />
                                            <label className="text-[16px] font-medium text-gray-700 dark:text-gray-300">
                                        ...
                                        {riptes.length > 0 ? (
                                            <select
                                                ...
                                                onChange={(e) => {
                                                    const ripte = riptes.find(r => r.id === parseInt(e.target.value));
                                            ...
                                            >
                                                ...
                                                {riptes.map((ripte) => (
                                                    <option key={ripte.id} value={ripte.id}>
                                                        {ripte.month} {ripte.year} - ${Number(ripte.value).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                    </option>
                                                ))}
                                            </select>
                                        ) : (
                                    ...
                                    <div>
                                        <div className="flex items-center gap-1 mb-1">
                                            <BarChart3 className="text-gray-500 dark:text-gray-400" size={12} />
                                            <label className="text-[16px] font-medium text-gray-700 dark:text-gray-300">
                                                IBM
                                            </label>
                                        </div>
                            ...
                            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 mb-3">
                                <div className="flex items-center gap-1.5 mb-2">
                                    <Calendar className="text-gray-600 dark:text-gray-400" size={14} />
                                    <h3 className="text-sm font-semibold text-gray-900 dark:text-white">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-1.5 mb-1.5">
                                            <Calendar className="text-green-500 dark:text-green-400" size={14} />
                                            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                                                Hasta
                                            </label>
                                        </div>
                                ...
                                <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-3 mb-3">
                                    ...
                                    {hayHaberesIngresados && (
                                        <div className="mb-2">
                                            ...
                                            >
                                                <Copy size={14} />
                                                <span className="text-sm">Replicar Primer Haber a Todas las Filas</span>
                                            </button>
                                        </div>
                                    )}
                                                    ...
                                                    <tbody className="bg-white dark:bg-gray-700">
                                                        {remuneraciones.map((row, index) => (
                                                            <tr key={index}>
                                                                ...
                                                                <td className="border border-gray-300 dark:border-gray-600 px-2 py-1 text-right text-[16px] text-gray-900 dark:text-white">
                                                                    {row.haber_ajustado ? `$ ${row.haber_ajustado.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'}
                                                                </td>
                                                            </tr>
                                                        ))}
                                                        <tr className="bg-gray-100 dark:bg-gray-600 font-semibold">
                                                            ...
                                                            <td className="border border-gray-300 dark:border-gray-600 px-2 py-1 text-right text-[16px] text-gray-900 dark:text-white">
                                                                $ {totalHaberes.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                            </td>
                                                            <td className="border border-gray-300 dark:border-gray-600 px-2 py-1"></td>
                                                            <td className="border border-gray-300 dark:border-gray-600 px-2 py-1 text-right text-[16px] text-gray-900 dark:text-white">
                                                                $ {totalHaberesAjustados.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                            </td>
                                                        </tr>
                                                        <tr className="bg-gray-100 dark:bg-gray-600 font-semibold">
                                                            <td className="border border-gray-300 dark:border-gray-600 px-2 py-1 text-center text-[16px] text-gray-900 dark:text-white">
                                                            ...
                                                            <td className="border border-gray-300 dark:border-gray-600 px-2 py-1"></td>
                                                            <td className="border border-gray-300 dark:border-gray-600 px-2 py-1 text-right text-[16px] text-gray-900 dark:text-white">
                                                                $ {promedioHaberesAjustados.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            ...
                                            <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
                                                <table className="min-w-full border border-gray-300 dark:border-gray-600">
                                                    <thead className="bg-teal-500 text-white sticky top-0">
                                                        <tr>
                                                            <th className="border border-gray-300 dark:border-gray-600 px-1 py-1.5 text-center text-[9px] uppercase">PERÍODO</th>
                                                            <th className="border border-gray-300 dark:border-gray-600 px-1 py-1.5 text-center text-[9px] uppercase">3M ATRÁS</th>
                                                            <th className="border border-gray-300 dark:border-gray-600 px-1 py-1.5 text-center text-[9px] uppercase">%</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody className="bg-white dark:bg-gray-700">
                                                        ...
                                                        <tr className="bg-teal-100 dark:bg-teal-900 font-semibold">
                                                            <td className="border border-gray-300 dark:border-gray-600 px-1 py-0.5 text-center text-[16px] text-gray-900 dark:text-white" colSpan={2}>
                                                                TOTAL
                                                            </td>
                                                            <td className="border border-gray-300 dark:border-gray-600 px-1 py-0.5 text-right text-[16px] text-gray-900 dark:text-white">
                                                                {(() => {
                                                                    const totalPercentage = porcentajesRipte.reduce((sum, row) => {
                                                                        if (row.riptePercentage !== null) {
                                                                            return sum + row.riptePercentage;
                                                                        }
                                                                        return sum;
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex justify-between items-center py-1 border-b border-gray-200 dark:border-gray-700">
                                                <span className="text-[16px] font-medium text-gray-700 dark:text-gray-300">IBM CON RIPTE:</span>
                                            ...
                                            <div className="flex justify-between items-center py-1 border-b border-gray-200 dark:border-gray-700">
                                                <span className="text-[16px] font-medium text-gray-700 dark:text-gray-300">TASA VARIACIÓN:</span>
                                                <span className="text-[16px] font-semibold text-gray-900 dark:text-white">
                                            ...
                                            <div className="flex justify-between items-center py-1 border-b border-gray-200 dark:border-gray-700">
                                                <span className="text-[16px] font-medium text-gray-700 dark:text-gray-300">IBM TOTAL:</span>
                                                <span className="text-[16px] font-semibold text-gray-900 dark:text-white">
                                                    $ {ibmTotal.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                </span>
                                            </div>

                                            <div className="flex justify-between items-center py-1 border-b border-gray-200 dark:border-gray-700">
                                                <span className="text-[16px] font-medium text-gray-700 dark:text-gray-300">% INCAPACIDAD</span>
                                                <span className="text-[16px] font-semibold text-gray-900 dark:text-white">
                                                    {disabilityPercentage !== null ? `${disabilityPercentage}%` : '0%'}
                                                </span>
                                            </div>

                                            <div className="flex justify-between items-center py-1 border-b border-gray-200 dark:border-gray-700">
                                                <span className="text-[16px] font-medium text-gray-700 dark:text-gray-300">COEF. EDAD</span>
                                                <span className="text-[16px] font-semibold text-gray-900 dark:text-white">
                                                    {customerAge !== null ? coeficienteEdad.toFixed(2) : '0'}
                                                </span>
                                            </div>
                                            ...
                                            <div className="flex justify-between items-center py-2 bg-blue-50 dark:bg-blue-900/30 px-2 rounded-md border border-blue-200 dark:border-blue-700">
                                                <span className="text-sm font-bold text-blue-900 dark:text-blue-100">TOTAL FÓRMULA</span>
                                                <span className="text-sm font-bold text-blue-900 dark:text-blue-100">
                                                    $ {totalFormula.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                </span>
                                            </div>                                            {/* Switch para activar 20% adicional */}
                                            <div className="flex items-center justify-between py-1.5 bg-gray-100 dark:bg-gray-800 px-2 rounded-md border border-gray-300 dark:border-gray-600">
                                                <span className="text-[16px] font-medium text-gray-700 dark:text-gray-300">Activar 20%</span>
                                                <label className="relative inline-flex items-center cursor-pointer">
                                                    <input
                                                        type="checkbox"
                                                        className="sr-only peer"
                                                        checked={activar20Porciento}
                                                        onChange={(e) => setActivar20Porciento(e.target.checked)}
                                                    />
                                            ...
                                            {activar20Porciento && (
                                                <div className="flex justify-between items-center py-1 border-b border-gray-200 dark:border-gray-700">
                                                    <span className="text-[16px] font-medium text-gray-700 dark:text-gray-300">20% TOTAL:</span>
                                                    <span className="text-[16px] font-semibold text-gray-900 dark:text-white">
                                                        $ {veintePorCiento.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                    </span>
                                                </div>
                                            )}

                                            {/* Total Indemnización */}
                                            <div className={`flex justify-between items-center py-2 px-2 rounded-md border ${activar20Porciento
                                                ? 'bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-700'
                                                : 'bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-700'
                                                }`}>
                                                <span className={`font-bold text-sm ${activar20Porciento
                                                ...
                                                <span className={`font-bold text-sm ${activar20Porciento
                                                    ...
                                                    $ {totalIndemnizacion.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                </span>
                                            </div>
                                                ...
                                                <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-700">
                                                    ...
                                                    <div className="space-y-1">
                                                        {interesesCalculados.map((item, index) => (
                                                            <div key={index} className="flex justify-between items-center py-1 border-b border-yellow-200 dark:border-yellow-600">
                                                                <div className="flex items-center gap-4">
                                                                    <span className="text-sm font-semibold text-yellow-900 dark:text-yellow-100 min-w-[50px]">
                                                                        {item.año}
                                                                    </span>
                                                                ...
                                                                <span className="text-sm font-semibold text-yellow-900 dark:text-yellow-100">
                                                                    $ {item.interesDelAño.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                                </span>
                                                            </div>
                                                        ))}
                                                        ...
                                                        <div className="flex justify-between items-center py-2 mt-2 bg-yellow-100 dark:bg-yellow-800 px-2 rounded border border-yellow-300 dark:border-yellow-600">
                                                            ...
                                                            <span className="text-sm font-bold text-yellow-900 dark:text-yellow-100">
                                                                $ {totalIntereses.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                            </span>
                                                        </div>
                                                        ...
                                                        <div className="flex justify-between items-center py-2 mt-1 bg-orange-100 dark:bg-orange-900 px-2 rounded border border-orange-300 dark:border-orange-600">
                                                            ...
                                                            <span className="text-sm font-bold text-orange-900 dark:text-orange-100">
                                                                $ {totalConIntereses.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="flex items-center gap-2 py-1.5 bg-gray-100 dark:bg-gray-800 px-2 rounded-md border border-gray-300 dark:border-gray-600">
                                                <span className="text-[16px] font-medium text-gray-700 dark:text-gray-300 min-w-[70px]">Piso Mín:</span>
                                                <input
                                                    type="number"
                                                    step="0.01"
                                                ...
                                                />
                                                <span className="text-[16px] font-semibold text-gray-900 dark:text-white min-w-[100px] text-right">
                                                    $ {totalPisoMinimo.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                </span>
                                            </div>

                                            {/* Switch para activar 20% del piso mínimo */}
                                            <div className="flex items-center justify-between py-1.5 bg-gray-100 dark:bg-gray-800 px-2 rounded-md border border-gray-300 dark:border-gray-600">
                                                <span className="text-[16px] font-medium text-gray-700 dark:text-gray-300">Activar Piso</span>
                                                <label className="relative inline-flex items-center cursor-pointer">
                                                    <input
                                                        type="checkbox"
                                                        className="sr-only peer"
                                                        checked={activarPisoMinimo}
                                                        onChange={(e) => setActivarPisoMinimo(e.target.checked)}
                                                    />
                                                    <div className="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                                                </label>
                                            </div>
                                            ...
                                            {activarPisoMinimo && totalPisoMinimo > 0 && (
                                                <div className="flex justify-between items-center py-1 border-b border-gray-200 dark:border-gray-700">
                                                    <span className="text-[16px] font-medium text-gray-700 dark:text-gray-300">20% PISO:</span>
                                                    <span className="text-[16px] font-semibold text-gray-900 dark:text-white">
                                                        $ {veintePorCientoPiso.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                    </span>
                                                </div>
                                            )}
                                            ...
                                            {totalPisoMinimo > 0 && (
                                                <div className={`flex justify-between items-center py-2 px-2 rounded-md border ${activarPisoMinimo
                                                    ? 'bg-purple-50 dark:bg-purple-900/30 border-purple-200 dark:border-purple-700'
                                                    : 'bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-600'
                                                    }`}>
                                                    <span className={`font-bold text-sm ${activarPisoMinimo
                                                    ...
                                                    <span className={`font-bold text-sm ${activarPisoMinimo
                                                        ...
                                                        $ {totalPisoMinimoFinal.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="bg-white dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                                            ...
                                            <div className="mt-3 space-y-2">
                                                ...
                                                <button
                                                    onClick={() => setShowSavedLiquidations(!showSavedLiquidations)}
                                                                    ...
                                                                    {savedLiquidations.map((liq: any) => (
                                                                        ...
                                                                        >
                                                                            <div className="flex items-start justify-between">
                                                                                ...
                                                                                <button
                                                                                    onClick={() => handleLoadLiquidation(liq.id)}
