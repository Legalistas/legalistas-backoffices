...
export default function CasesDetailsPage() {
	...
	useEffect(() => {
		if (cases?.files && cases.files.length > 0) {
			...
			const maxStage = Math.max(
				...cases.files.map((file) => file.proceduralStageId || 1),
			);
	...
	const fetchCaseData = useCallback(async () => {
		try {
			...
			setError(
				`Failed to load case data: ${(err as Error).message || "Unknown error"}`,
			);
			toast.error(
				`Error al cargar el caso: ${(err as Error).message || "Error desconocido"}`,
			);
		} finally {
			setIsLoading(false);
		}
	...
	const filteredFiles = useCallback(() => {
		...
		return fileTypeFilter === 0
			? cases?.files
			: cases?.files.filter((exp) => exp.filetype === fileTypeFilter);
			...
			try {
				...
				const errorMessage =
					error instanceof Error
						? error.message
						: "Error desconocido al actualizar el caso";
