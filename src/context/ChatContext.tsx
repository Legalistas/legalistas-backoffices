...
export const ChatProvider: React.FC<{
	...
	const markMessagesAsRead = useCallback(async () => {
		...
		const hasUnreadMessages = recentChats.some(
			(chat) => chat.user.id === activeChat.id && chat.unreadCount > 0,
		);

		if (!hasUnreadMessages) return;

		try {
			await fetchWithAuth(CHAT_READ_ENDPOINT(activeChat.id), {
				method: "POST",
			});
			...
			setMessages((prev) =>
				prev.map((msg) =>
					msg.senderId === activeChat.id && !msg.isRead
						? { ...msg, isRead: true }
						: msg,
				),
			);
			...
			setRecentChats((prev) =>
				prev.map((chat) =>
					chat.user.id === activeChat.id ? { ...chat, unreadCount: 0 } : chat,
				),
			);
		} catch (err) {
					...
					return [
						{
							...
							lastMessage: message,
							unreadCount:
								message.senderId !== userId && !message.isRead ? 1 : 0,
						},
						...prev,
					];
				}
	...
	useEffect(() => {
		...
		if (!socketRef.current) {
			...
			socket.io.engine.on("upgradeError", (error) => {
				...
				console.log(
					"Continuing with current transport:",
					socket.io.engine.transport.name,
				);
			});
	...
	const handleSetActiveChat = async (user: User) => {
		...
		try {
			const chatMessages = await fetchWithAuth<ChatMessage[]>(
				CHAT_HISTORY_ENDPOINT(user.id),
			);
	...
	const sendMessage = useCallback(
		async (content: string) => {
			if (!socketRef.current || !activeChat || !content.trim() || isSending)
				return;
			...
			try {
				...
				const newMessage = await fetchWithAuth<ChatMessage>(
					CHAT_MESSAGES_ENDPOINT,
					{
						method: "POST",
						body: JSON.stringify({
	...
	const startTyping = useCallback(() => {
		if (!socketRef.current || !activeChat || !socketRef.current.connected)
			return;
