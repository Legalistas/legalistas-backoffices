...
import { FileText, Download, Trash2, File, FileImage, FileIcon as FilePdf, FileArchive, FileCode } from "lucide-react"
...
import { BASE_URL, LEADS_DOCUMENTS_DELETE_ENDPOINT } from "@/constant/api-endpoints"
...
export default function LeadDocuments({ lead, onLeadUpdate }: LeadDocumentsProps) {
    ...
    const handleDelete = async (documentId: number) => {
        ...
        try {
            ...
            if (onLeadUpdate) {
                ...
                const updatedLead = {
                    ...lead,
                    crmDocument: lead.crmDocument?.filter((doc) => doc.id !== documentId) || [],
                }
                                    ...
                                    <TableRow key={document.id} className="hover:bg-gray-50">
                                        ...
                                        <TableCell className="px-4 py-3 text-sm text-center">
                                            <div className="flex justify-center space-x-2">
                                                <Button variant="custom" size="sm" onClick={() => handleDownload(document)} className="hover:bg-gray-100 transform transition duration-200 ease-in-out p-2 rounded-full">
