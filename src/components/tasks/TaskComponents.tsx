...
function EmptyState({
	...
	return (
		<div className="flex flex-col items-center justify-center py-16 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-white/3">
			...
			<Button variant="default" size="sm" className="mt-4" onClick={onNewTask}>
				Nueva Tarea
			</Button>
		</div>
	);
...
export default function TaskComponents() {
	...
	const fetchTasks = useCallback(async () => {
		...
		try {
			...
			const res = await fetch(`${TASKS_ENDPOINT}?${params}`, {
				headers: headers(),
			});
			if (res.ok) {
			...
			<div className="flex flex-wrap items-center gap-3">
				<div className="relative flex-1 min-w-50 max-w-md">
					...
					<input
						...
						className="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-white/3 py-2 pl-9 pr-3 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 outline-none focus:border-primary"
					/>
				</div>
				<select
					value={priority}
					onChange={(e) => setPriority(e.target.value)}
					className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-white/3 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 outline-none focus:border-primary"
				>
				...
				<select
					value={caseFilter ?? ""}
					onChange={(e) =>
						setCaseFilter(e.target.value ? Number(e.target.value) : undefined)
					}
					className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-white/3 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 outline-none focus:border-primary max-w-50"
				>
							...
							>
								...
								<div className="flex items-center gap-3 px-4 py-3">
									...
									<div className="flex-1 min-w-0">
										<p
											className={`text-sm font-semibold ${task.completed ? "line-through text-gray-400" : "text-gray-900 dark:text-white"}`}
										>
											{task.title}
										</p>
										<div className="flex flex-wrap items-center gap-2 mt-1">
											{/* Priority badge */}
											<span
												className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium ${PRIORITY_COLORS[task.priority] ?? "bg-gray-100 text-gray-600"}`}
											>
											...
											{STATUS_BADGES[task.status] && (
												<span
													className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-medium ${STATUS_BADGES[task.status].className}`}
												>
											...
											{task.case && (
												<a
													href={`/admin/legal-cases/${task.case.id}`}
													className="text-xs text-blue-600 dark:text-blue-400 hover:underline"
												>
													{task.case.number
														? `${task.case.number} - ${task.case.title}`
														: task.case.title}
												</a>
											)}
										</div>
										...
										<div>
												...
												<ul className="space-y-1.5 mb-2">
													{task.subTasks.map((sub) => (
														...
														>
															...
															/>
															<span
																className={`text-sm ${sub.completed ? "line-through text-gray-400" : "text-gray-700 dark:text-gray-300"}`}
															>
																{sub.title}
															</span>
														</li>
											...
											<div className="flex items-center gap-2">
												<input
													...
													className="flex-1 rounded-lg border border-gray-200 dark:border-gray-700 bg-transparent px-3 py-1.5 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 outline-none focus:border-primary"
												/>
												<button
