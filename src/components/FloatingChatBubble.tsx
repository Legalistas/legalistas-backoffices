...
export default function FloatingChatBubble({ position = "bottom-right", offset = 20 }: FloatingChatBubbleProps) {
    ...
    useEffect(() => {
        ...
        console.log("🔔 BUBBLE: Checking unread messages", {
            totalUnread,
    ...
    return (
        <div className="fixed z-9999" style={positionStyles} data-testid="floating-chat-bubble">
                        ...
                        <div className="space-y-2 max-h-60 overflow-y-auto">
                            {recentChats.length > 0 ? (
                                recentChats.slice(0, 3).map((chat) => (
                                    <div
                                        key={chat.user.id}
                                        className={`flex items-center gap-2 p-2 rounded-md cursor-pointer transition-all ${chat.unreadCount > 0
                                                ? "bg-brand-50 hover:bg-brand-100 border border-brand-200 shadow-sm"
            ...
            <motion.button
                ...
                className="relative flex items-center justify-center w-14 h-14 rounded-full bg-brand-500 hover:bg-brand-600 text-black shadow-lg transition-colors"
