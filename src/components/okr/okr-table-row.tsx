...
export function OKRTableRow({
	...
	return (
		<TableRow
			key={item.id}
			className="group border-b border-border bg-card hover:bg-secondary/50 transition-colors"
		>
			{showUnidad && (
				<TableCell
					rowSpan={rowSpan}
			...
			<TableCell className="border-r border-border px-2 py-1.5">
				<ProgressBar
					value={item.gradoAvance}
					onChange={(v) => onUpdate(item.id, "gradoAvance", v)}
				/>
			</TableCell>
			<TableCell className="border-r border-border px-2 py-1.5">
				<DatePickerCell
					date={item.fechaObjetivo}
					onChange={(d) => onUpdate(item.id, "fechaObjetivo", d)}
				/>
			</TableCell>
