...
const stageConfig: Record<
	number,
	{ bg: string; color: string; borderColor: string; icon: typeof FileText }
> = {
	1: {
		bg: "bg-sky-50",
		color: "text-sky-700",
	...
	return (
		<DragDropContext onDragEnd={handleDragEnd}>
			<div className="relative flex-1">
				...
				<div className="flex gap-4 pb-4 overflow-x-auto h-[calc(100vh-280px)] [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:hover:bg-gray-400">
					{stageCases.map((stage) => {
						const config = stageConfig[stage.value] ?? defaultConfig;
						const Icon = config.icon;
						const stageCasesList = cases.filter(
							(c) => c.stageId === stage.value,
						);
						...
						return (
							...
							>
								{/* Column header */}
								<div
									className={`p-3 border-b ${config.borderColor} dark:border-gray-700`}
								>
												...
												>
													{(provided, snapshot) => (
														...
														>
															...
															{/* Title */}
															<h4 className="text-sm font-medium text-gray-900 dark:text-white mb-1 line-clamp-2 uppercase">
