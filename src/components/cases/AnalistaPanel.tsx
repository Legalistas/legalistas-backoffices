...
import {
	DASHBOARD_LEGAL_STATS_ENDPOINT,
	LEXIA_ANALYTICS_ENDPOINT,
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-xs text-gray-500 dark:text-gray-400">
								Análisis inteligente de tu estudio
							</p>
						</div>
					</div>
					<button
						...
						<div className="rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800/30 p-3">
							<div className="flex items-center gap-2 mb-1">
								...
								<span className="text-lg font-bold text-green-600 dark:text-green-400">
									{stats.audiencias_proximas_7d}
								</span>
							</div>
							<p className="text-[11px] text-green-500 dark:text-green-400 font-medium">
								Audiencias 7d
							</p>
						</div>
						<div className="rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-3">
						...
						<>
							<div className="flex flex-col items-center text-center mt-6 mb-8">
								...
								<p className="text-xs text-gray-500 dark:text-gray-400 max-w-[260px]">
									Consultá sobre tus casos, plazos, finanzas, audiencias y más.
									Todos los datos son reales de tu estudio.
								</p>
							</div>
											...
											{msg.funciones && msg.funciones.length > 0 && (
												<div className="flex flex-wrap items-center gap-1.5 px-1">
													{msg.funciones.map((fn, j) => (
														<span
															key={j}
															className="inline-flex items-center gap-1 text-[11px] bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 px-2.5 py-1 rounded-md font-medium"
														>
															{fn}
														</span>
													))}
				...
				<div className="px-5 py-4 border-t border-gray-100 dark:border-gray-800">
					<div className="flex items-center gap-2">
						<input
							...
							disabled={isLoading}
							className="flex-1 px-3 py-2.5 text-sm border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#09A4B5]/30 focus:border-primary disabled:opacity-50"
						/>
						<button
							onClick={handleSend}
							disabled={!query.trim() || isLoading}
							className="h-10 w-10 flex items-center justify-center rounded-lg bg-primary text-white hover:bg-primary/80 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shrink-0"
						>
							{isLoading ? (
					...
					<p className="text-[10px] text-gray-400 dark:text-gray-500 text-center mt-2">
						Los datos provienen directamente de tu estudio
					</p>
				</div>
			</div>
		</>
