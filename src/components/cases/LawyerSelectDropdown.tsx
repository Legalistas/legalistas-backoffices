...
export function LawyerSelectDropdown({
    ...
    const getLawyerImgSrc = (image?: string) => {
        ...
        return image.startsWith("http") ? image : `${process.env.NEXT_PUBLIC_BACKEND_URL}${image}`
    }
                        ...
                        <div className="overflow-auto py-1 max-h-52">
                            {isLoading ? (
                                ...
                                <div className="px-3 py-2 text-xs text-gray-500">
                                    {lawyers.length === 0 ? "No hay abogados disponibles" : "Sin resultados"}
                                </div>
                            ) : (
                                filteredLawyers.map((lawyer) => (
                                    <button
                                        ...
                                        className={`w-full px-3 py-2 text-left text-sm hover:bg-gray-50 transition-colors flex items-center gap-2 ${lawyer.id === currentLawyerId ? "bg-gray-50 font-medium" : ""
                                            }`}
                                    >
                                        {getLawyerImgSrc(lawyer.image) ? (
                                            ...
                                            <div className={`w-5 h-5 rounded-full shrink-0 flex items-center justify-center text-[10px] font-medium ${type === "responsible"
        ...
        <div className="relative">
            ...
            >
                {displayImgSrc ? (
                    ...
                    <div className={`w-5 h-5 rounded-full shrink-0 flex items-center justify-center text-[10px] font-medium bg-${accentColor}-100 text-${accentColor}-600`}>
                        {currentLawyerName?.charAt(0).toUpperCase() ?? "?"}
                    </div>
                )}
                <span className="text-sm text-gray-700">{currentLawyerName || "Sin asignar"}</span>
