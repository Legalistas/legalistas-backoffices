...
export function SalesSource() {
		...
		const fetchData = async () => {
			try {
				...
				const response = await fetch(
					`${STATISTICS_CRM_DASHBOARD_SC_ENDPOINT}/${session?.user?.id}`,
					{
						headers: {
	...
	const hasSocialData =
		data?.monthlySocialData && data.monthlySocialData.length > 0;
			...
			<CardHeader>
				<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
					<div>
						...
						<CardDescription>
							Análisis de canales de ingreso de leads
						</CardDescription>
					</div>
					<div className="flex items-center gap-2 mt-2 sm:mt-0">
						...
						<Switch
							...
							onCheckedChange={(checked) =>
								setActiveView(checked ? "social" : "general")
							}
								...
								<div className="grid grid-cols-1 gap-6 md:grid-cols-2">
									<div>
										<h3 className="mb-4 text-sm font-medium">
											Distribución de Leads por{" "}
											{activeView === "social" ? "Red Social" : "Fuente"}
										</h3>
									...
									<div>
										...
										<div className="h-[300px] w-full">
											{typeof window !== "undefined" && (
												<ReactApexChart
													options={
														conversionChartData.options as ApexCharts.ApexOptions
													}
						...
						{activeView === "social" && !hasSocialData && (
							<div className="mt-4 rounded-md bg-muted p-4 text-center">
								<p className="text-muted-foreground">
									No hay datos disponibles para redes sociales.
								</p>
							</div>
						)}
					</>
