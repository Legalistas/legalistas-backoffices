		...
		<div className="fixed bottom-4 right-4 bg-gray-800 text-white p-3 rounded-lg text-xs font-mono z-50 shadow-lg">
			...
			{connectionInfo && (
				<>
					<div>
						Transport:{" "}
						<span className="text-blue-400">{connectionInfo.transport}</span>
					</div>
					<div>
						Socket ID:{" "}
						<span className="text-gray-400">{connectionInfo.socketId}</span>
					</div>
					<div>
						Ping:{" "}
						<span className="text-green-400">{connectionInfo.ping}ms</span>
					</div>
				</>
			)}
