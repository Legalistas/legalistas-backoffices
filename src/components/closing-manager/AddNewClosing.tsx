...
export default function AddNewClosing({
	open,
	onOpenChange,
	onSuccess,
}: AddNewClosingProps) {
	...
	const selectedNegotiation =
		finalizedNegotiations.find(
			(n) => n.id.toString() === selectedNegotiationId,
		) || null;
	const acceptedOffer =
		selectedNegotiation?.offers.find((o) => o.accepted) || null;
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!selectedNegotiationId || !acceptedOffer) {
			toast.error("Selecciona una negociación con oferta aceptada");
			return;
		}
		setIsLoading(true);
		try {
			...
			toast.error(err instanceof Error ? err.message : "Error al crear el cierre");
		} finally {
			setIsLoading(false);
		}
	};
			...
			<DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					<DialogTitle className="text-xl">
						Nuevo Cierre desde Negociación
					</DialogTitle>
					<DialogDescription className="sr-only">Formulario para crear un nuevo cierre desde negociación</DialogDescription>
				</DialogHeader>
					...
					<div className="space-y-2">
						...
						{loadingNegotiations ? (
							...
							<Select
								...
								onValueChange={setSelectedNegotiationId}
							>
								...
								<SelectContent>
									{finalizedNegotiations.length === 0 ? (
										<div className="p-2 text-sm text-gray-500">
											No hay negociaciones finalizadas
										</div>
									) : (
							...
							<div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
								...
								<div>
									<span className="text-gray-500">
										Capital (oferta aceptada):
									</span>{" "}
									<span className="font-medium text-green-600">
										{formatARS(Number(acceptedOffer.amount))}
									</span>
								</div>
								<div>
