...
const RoleMultiSelect: React.FC<RoleMultiSelectProps> = ({
    ...
    const removeOption = (valueToRemove: string, event?: React.MouseEvent) => {
        ...
        const newSelectedOptions = selectedOptions.filter((value) => value !== valueToRemove)
        setSelectedOptions(newSelectedOptions)
        if (onChange) onChange(newSelectedOptions)
    }
                                ...
                                {options.length > 0 ? (
                                    options.map((option) => (
                                        <div key={`option-${option.value}`}>
                                            ...
                                            >
                                                <div
                                                    className={`relative flex w-full items-center p-3 ${selectedOptions.includes(option.value) ? "bg-blue-50 dark:bg-blue-900/20" : ""
                                                        }`}
                                                >
                                                    <div className="flex items-center">
                                                        <input
                                                            ...
                                                            readOnly
                                                        />
                                                        <div className="leading-6 text-gray-800 dark:text-white/90">{option.text}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="p-3 text-gray-500 dark:text-gray-400">No hay roles disponibles</div>
                                )}
                            </div>
                        </div>
                    )}
