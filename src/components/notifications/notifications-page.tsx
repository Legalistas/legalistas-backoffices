...
import { RefreshCw, Inbox, CheckCircle, AlertTriangle, Bell } from "lucide-react"
import type { Notification } from "@/components/notification-provider"

// Tipos para los filtros
type FilterType = "all" | "unread" | "read"
type SortType = "newest" | "oldest"
type NotificationType = "all" | "crm" | "cases" | "cause_files" | "events" | "system" | "info" | "success" | "warning" | "error"

export default function NotificationsPage() {
    const { notifications, loading, error, refreshNotifications, markNotificationAsRead, currentUserId } =
        useNotifications()
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h2 className="text-2xl font-medium text-red-500 mb-2">Error al cargar notificaciones</h2>
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex flex-col gap-6 mb-6">
                ...
                <NotificationFilters
                    ...
                    setNotificationType={(value: string) => setNotificationType(value as NotificationType)}
                    searchQuery={searchQuery}
                    setSearchQuery={setSearchQuery}
                />
            </div>

            <Tabs defaultValue="all" value={filterType} onValueChange={(value) => setFilterType(value as FilterType)} className="w-full bg-white dark:bg-gray-800">
...
function renderNotificationList(notifications: Notification[], loading: boolean, handleMarkAllAsRead: () => void) {
    if (loading) {
        return (
    ...
    if (notifications.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-muted-foreground">No se encontraron notificaciones con los filtros seleccionados.</p>
            </div>
        )
    }
