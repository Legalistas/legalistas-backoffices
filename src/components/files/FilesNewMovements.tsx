...
export default function FilesNewMovements({
	isOpen,
	onClose,
	file,
	onFileUpdate,
}: FilesNewMovementsProps) {
	...
	const [observationAttachment, setObservationAttachment] =
		useState<string>("");
	...
	useEffect(() => {
		...
		if (file?.case?.responsibleLawyer?.id) {
			console.log(
				"Setting responsiblePerson to:",
				file.case.responsibleLawyer.id,
			);
	...
	useEffect(() => {
		if (selectedType) {
			const typeObj = FILES_MOVEMENTS_TYPE.find(
	...
	const handleNewMovement = async () => {
		...
		if (!selectedType) {
			toast.error("Por favor seleccione un tipo de movimiento");
			return;
		}

		if (hasSubTypes && !selectedSubType) {
			toast.error("Por favor seleccione un subtipo de movimiento");
			return;
		}
		...
		if (!responsiblePerson) {
			toast.error("Por favor seleccione un responsable");
		...
		try {
			...
			console.log(
				"Token:",
				session?.user?.accessToken ? "Presente" : "Faltante",
			);
		...
		<Modal isOpen={isOpen} onClose={onClose} className="max-w-xl">
			<div className="p-6">
				...
				<p className="text-muted-foreground mb-6">
					Agregar un evento al expediente{" "}
				...
				<div className="space-y-4 mb-6">
					...
					<div
						className={`grid gap-4 ${hasSubTypes ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1"}`}
					>
						{/* Selector de tipo */}
								...
								{responsibleLawyer && (
									<label className="flex items-center px-2 py-1 border rounded-md cursor-pointer hover:bg-gray-50 text-sm w-full">
										<input
											...
											checked={
												responsiblePerson === String(responsibleLawyer.id)
											}
					...
					<div className="flex flex-col space-y-2 mt-4">
						<label
							htmlFor="observation-attachment"
							className="font-medium text-sm"
						>
							Observaciones
						</label>
				...
				<div className="flex justify-end space-x-2">
					...
					<Button
						...
						variant="default"
						className="bg-primary text-white px-4 py-2.5 text-sm hover:bg-primary/85 dark:text-gray-900 disabled:opacity-50 disabled:cursor-not-allowed"
					>
