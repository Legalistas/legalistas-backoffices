...
import { Loader2, Send, Trash2, Clock, MessageSquare } from "lucide-react"
import { toast } from "sonner"
import { CASES_FILES_NOTES_CREATE_ENDPOINT, CASES_FILES_NOTES_DELETE_ENDPOINT } from "@/constant/api-endpoints"
...
export default function FilesNotes({ fileId, caseId, notes = [], onNotesChange, onRefresh }: FilesNotesProps) {
    ...
    const handleAddNote = async (e: React.FormEvent) => {
        ...
        setIsSending(true)
        try {
            const response = await fetch(CASES_FILES_NOTES_CREATE_ENDPOINT(caseId, fileId), {
                method: "POST",
                headers: {
            ...
            toast.error(error instanceof Error ? error.message : "Error al agregar la nota")
        } finally {
            setIsSending(false)
        }
    }
    ...
    const handleDeleteNote = async (noteId: number) => {
        ...
        try {
            const response = await fetch(`${CASES_FILES_NOTES_DELETE_ENDPOINT(caseId, fileId, noteId)}`, {
                method: "DELETE",
                headers: {
                    Authorization: `Bearer ${session.user.accessToken}`,
                },
            ...
            <div className="space-y-4 max-h-96 overflow-y-auto">
                {isLoading ? (
                            ...
                            >
                                <div className="flex justify-between items-start mb-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="font-medium text-gray-900 dark:text-gray-100">{note.user?.name || "Usuario"}</div>
                                ...
                                <div className="text-gray-800 dark:text-gray-200 whitespace-pre-wrap leading-relaxed">{note.note}</div>
                                {note.updatedAt && note.updatedAt !== note.createdAt && (
                                    <div className="mt-2 text-xs text-gray-400 italic">Editado {formatDate(note.updatedAt)}</div>
                                )}
                            </div>
                        ))
                ) : (
                    <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                        ...
                        <p className="text-sm">Agrega la primera nota interna para este archivo</p>
                    </div>
                )}
            </div>
