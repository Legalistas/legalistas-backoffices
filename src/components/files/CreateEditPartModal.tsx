...
export default function CreateEditPartModal({
	...
	const fetchClients = useCallback(async () => {
		...
		try {
			...
			const customersWithRole35 = Array.isArray(data.data)
				? data.data
						.filter((customer: any) =>
							customer.roleUser?.some((ru: any) => ru.role.id === 35),
						)
						.filter(
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			toast.success(
				part ? "Parte actualizada exitosamente" : "Parte agregada exitosamente",
			);
	...
	const handleUseSelectedClient = async () => {
		...
		try {
			...
			toast.success(
				`ART "${selectedClient.name}" agregada exitosamente al caso`,
			);
				...
				<div className="p-3 sm:p-4 md:p-6">
					<div className="mb-4 sm:mb-6">
						...
						<p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
							{part
								? "Modifique la información de la parte"
								: "Busque un ART existente o cree una nueva parte"}
						</p>
					</div>
								...
								<div className="flex items-center gap-2 mb-3 sm:mb-4">
									...
									<div>
										...
										<p className="text-sm text-gray-500 dark:text-gray-400">
											Busque y seleccione un cliente existente para agregarlo
											como parte del caso
										</p>
									</div>
								</div>
															...
															<div className="flex-1 min-w-0">
																...
																{client.userAddresses?.length && (
																	<p className="text-xs text-gray-400 truncate">
																		Dirección:{" "}
																		{client.userAddresses[0].street +
																			" " +
																			client.userAddresses[0].streetNumber}{" "}
																		- {client.userAddresses[0].city} -{" "}
																		{client.userAddresses[0].state.name}
																	</p>
																)}
															</div>
															{selectedClient?.id === client.id && (
