		...
		<Card className="p-4">
			...
			<CardContent>
				<div className="space-y-4">
					...
					<Separator />
					<div>
						{file?.instanceExpiration &&
							Number(file.instanceExpiration) > 0 &&
							(file?.lastMovementDate || file?.startDate) &&
							(() => {
								...
								expirationDate.setMonth(
									expirationDate.getMonth() + expirationMonths,
								);
								...
								return (
									<div className="flex flex-col gap-1">
										<span
											className={`px-2 py-1 rounded flex items-center gap-1 font-semibold ${colorClass}`}
										>
										...
										{file.lastMovementDate && (
											<span className="text-xs text-gray-500 ml-1">
												Último movimiento:{" "}
												{new Date(file.lastMovementDate).toLocaleDateString(
													"es-AR",
												)}
											</span>
										)}
