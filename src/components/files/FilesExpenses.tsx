...
import { ChevronDown, DollarSign, Eye, EyeOff, Loader2, Plus, Settings, Pencil, Trash2 } from "lucide-react"
import { Table, TableBody, TableCell, TableHeader, TableRow } from "../ui/table"
import CreateEditExpensesModal, { type FileExpenses as ModalFileExpenses } from "./CreateEditExpensesModal" // Import the modal and its specific type
import { toast } from "sonner"
import { CASES_FILES_EXPENSES_CREATE_ENDPOINT, CASES_FILES_EXPENSES_UPDATE_ENDPOINT, CASES_FILES_EXPENSES_DELETE_ENDPOINT } from "@/constant/api-endpoints" // Assuming this is for creating expenses
...
export default function FilesExpenses({
    ...
    const [selectedExpense, setSelectedExpense] = useState<FileExpenses | null>(null)
    const [openCreateEditExpenseModal, setOpenCreateEditExpenseModal] = useState(false)
    ...
    const toggleColumn = (key: keyof FileExpenses | "actions") => {
        setColumns((prev) =>
            prev.map((col) => (col.key === key && !col.required ? { ...col, visible: !col.visible } : col)),
        )
    }
    ...
    const handleSaveExpense = async (modalExpenseData: ModalFileExpenses) => {
        ...
        try {
            ...
            if (modalExpenseData.id) {
                ...
                method = "PATCH"
                endpoint = CASES_FILES_EXPENSES_UPDATE_ENDPOINT(caseId, fileId, modalExpenseData.id)
                payload = {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || `Error al ${modalExpenseData.id ? "actualizar" : "crear"} el gasto`)
            }
            ...
            if (modalExpenseData.id) {
                ...
                setExpenseParts((prev) => prev.map((exp) => (exp.id === updatedExpenseData.id ? updatedExpenseData : exp)))
    ...
    const renderCellContent = (expense: FileExpenses, columnKey: keyof FileExpenses | "actions") => {
        switch (columnKey) {
            case "id":
                return expense.id || "-"
            case "amount":
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">No hay gastos disponibles</span>
                    </div>
                    <button
                ...
                <div className="overflow-auto rounded-md border border-gray-200 bg-white dark:bg-gray-800">
                    <div className="text-center py-6">
                        ...
                        <p className="mt-2 text-muted-foreground">No hay gastos disponibles</p>
                    </div>
                </div>
    ...
    return (
        <div className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                    <div className="relative">
                        ...
                        >
                            ...
                            Configurar columnas
                            <ChevronDown className={`h-4 w-4 transition-transform ${showColumnConfig ? "rotate-180" : ""}`} />
                        </button>
                        ...
                        {showColumnConfig && (
                            <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-md shadow-lg z-10">
                                <div className="p-4">
                                    ...
                                    <div className="mb-4">
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Configuraciones predefinidas:</p>
                                        <div className="grid grid-cols-2 gap-2">
                                            {Object.entries(presetConfigurations).map(([key, preset]) => (
                                                <button
                                                    key={key}
                                                    onClick={() => applyPreset(key as keyof typeof presetConfigurations)}
                                    ...
                                    <div className="border-t border-gray-200 dark:border-gray-600 pt-3">
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Columnas individuales:</p>
                                            ...
                                            {columns.map((column) => (
                                                ...
                                                >
                                                    ...
                                                    <span className="text-gray-700 dark:text-gray-300">
                                                        {column.label}
                                                        {column.required && <span className="text-xs text-gray-400 ml-1">(requerido)</span>}
                                                    </span>
                                                    {column.visible ? (
                    ...
                    >
                        {isLoading ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <Plus className="h-4 w-4" />}
                        Agregar nuevo gasto
                    </button>
                </div>
            </div>
                        ...
                        <TableRow>
                            {visibleColumns.map((column) => (
                                <TableCell
                                    key={column.key}
                                    isHeader
                                    className={`px-4 py-3 text-sm font-semibold text-gray-700 dark:text-gray-300 text-left ${column.key === "id" ? "w-[1%]" : ""
                    ...
                    <TableBody>
                        {localExpenses.map((expense, index) => (
                            <TableRow key={expense.id ?? `row-${index}`}>
                                {visibleColumns.map((column) => (
                                    <TableCell key={column.key} className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400">
                Mostrando {visibleColumns.length - 1} de {columns.length - 1} columnas disponibles
            </div>
            <CreateEditExpensesModal
                open={openCreateEditExpenseModal}
                onClose={() => setOpenCreateEditExpenseModal(false)}
