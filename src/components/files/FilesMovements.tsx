...
import { Calendar, Clock, User, FileText, Video, Users, AlertTriangle, CheckCircle, XCircle, RefreshCw } from 'lucide-react'
import { FILES_MOVEMENTS_TYPE, FILES_MOVEMENTS_MODE, FILES_MOVEMENTS_STATUS_OPTIONS } from '@/constant/causes'
...
const getMovementSubTypeLabel = (type: number, subType?: number | null): string => {
    if (!subType) return ''
    const movementType = FILES_MOVEMENTS_TYPE.find((t) => t.value === type)
...
const getStatusLabel = (status: string): string => {
    const statusObj = FILES_MOVEMENTS_STATUS_OPTIONS.find((s) => s.value === status)
...
export default function FilesMovements({ movements, onNewMovement }: FilesMovementsProps) {
        ...
        return (
            <div className="text-center py-12">
                ...
                <p className="text-gray-500 dark:text-gray-400 mb-4">
                    Agrega un nuevo movimiento para comenzar a rastrear la actividad del expediente.
                </p>
                {onNewMovement && (
                    <button
    ...
    return (
        <div className="space-y-4">
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                        {movements.map((movement, index) => {
                            ...
                            const subTypeLabel = getMovementSubTypeLabel(movement.type, movement.subType)
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        {movementDate.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' })}
                                    </td>
                                    <td className="px-4 py-3 whitespace-nowrap">
                                        <div className="flex items-center gap-2">
                                            {getTypeIcon(movement.type)}
                                            <div>
                                                <span className={`text-sm font-medium ${isUrgent ? 'text-red-600' : 'text-gray-900 dark:text-gray-100'}`}>
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap">
                                        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(movement.status)}`}>
            ...
            <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Tipos de movimiento:</p>
