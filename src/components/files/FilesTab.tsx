...
export default function FilesTab({ file, onRefresh }: FilesHeaderProps) {
    ...
    return (
        <Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="parts" className="space-y-4 bg-white dark:bg-gray-800 text-gray-dark dark:text-gray-100">
                {file?.caseId && file?.id ? (
                    <FilesParts
                    ...
                    <div className="text-center py-8 text-gray-500">
                        <p>No se puede cargar las partes. Faltan datos del caso o archivo.</p>
                    </div>
                )}
            </TabsContent>
            <TabsContent value="expenses" className="space-y-4 bg-white dark:bg-gray-800 text-gray-dark dark:text-gray-100">
                {file?.caseId && file?.id ? (
                    // <FilesParts
                    //     parts={file?.filesParts}
                    //     caseId={file.caseId}
                    ...
                    <div className="text-center py-8 text-gray-500">
                        <p>No se puede cargar los gastos. Faltan datos del caso o archivo.</p>
                    </div>
                )}
            </TabsContent>
            <TabsContent value="notes" className="space-y-4 bg-white dark:bg-gray-800 text-gray-dark dark:text-gray-100">
                {file?.caseId && file?.id ? (
                    <FilesNotes
                        fileId={file.id}
                        caseId={file.caseId}
                    ...
                    <div className="text-center py-8 text-gray-500">
                        <p>No se puede cargar las notas. Faltan datos del caso o archivo.</p>
                    </div>
                )}
            </TabsContent>
