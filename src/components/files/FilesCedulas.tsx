...
import { CASES_FILES_CEDULAS_ENDPOINT, CASES_FILES_CEDULAS_BY_ID_ENDPOINT } from '@/constant/api-endpoints'
...
export default function FilesCedulas({
    ...
    const personalizeTemplate = (template: string, customJuez?: string, customSecretaria?: string) => {
        ...
        if (fileData?.court) {
            ...
            const ciudad = jurisdictionParts[jurisdictionParts.length - 1] || 'Rafaela' // Última parte: ciudad
            ...
            const firstPartName = (Array.isArray(fileData?.filesParts) && fileData.filesParts.length > 0)
                ? fileData.filesParts[0]?.name
                : ''

            const processType = getProcessTypeLabel(typeof fileData?.typeProcessId === 'number' ? fileData.typeProcessId : null)
            ...
            const secretariaValue = customSecretaria || secretary || fileData.secretary || '[COMPLETAR DATO]'
    ...
    const handleCedulaTypeChange = (type: string) => {
        ...
        setNewCedula({
            ...newCedula,
            cedulaType: type,
            content: personalizeTemplate(baseTemplate, newCedula.juez, newCedula.secretaria)
        })
    }
    ...
    const handleJuezChange = (juez: string) => {
        ...
        if (newCedula.partId) {
            const selectedPart = parts.find(p => p.id.toString() === newCedula.partId)
            if (selectedPart) {
    ...
    const applyPartDataToContent = (content: string, selectedPart: any): string => {
        ...
        const city = selectedPart.city && selectedPart.city !== '0'
        ...
        const province = selectedPart.stateId ? (stateNames[selectedPart.stateId] || 'SANTA FE') : 'SANTA FE'
    ...
    const handleAddCedula = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                toast.error(`Error al crear cédula: ${error.message || 'Error desconocido'}`)
            }
        } catch (error) {
        ...
        <div className="space-y-4">
            ...
            {isAddingCedula && (
                <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700 space-y-6">
                    <h4 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Nueva Cédula</h4>
