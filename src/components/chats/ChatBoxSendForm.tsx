...
// Lista simple de emojis populares
const EMOJIS = [
  '😀', '😃', '😄', '😁', '😊', '😍', '🥰', '😘', '🤗', '🤔',
  '😐', '😑', '😶', '🙄', '😏', '😣', '😥', '😮', '🤐', '😯',
  '😪', '😫', '🥱', '😴', '😌', '😛', '😜', '😝', '🤤', '😒',
...
export default function ChatBoxSendForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Verificar si el mensaje está vacío o si ya se está enviando
    if (!message.trim() || isSubmitting) return

    // Verificar si ha pasado suficiente tiempo desde el último envío (500ms)
    const now = Date.now()
    if (now - lastSubmitTimeRef.current < 500) {
      console.log("Preventing rapid message send")
      return
    }
  ...
  return (
    <div className="sticky bottom-0 p-3 border-t border-gray-200 dark:border-gray-800">
      ...
      {showEmojiPicker && (
        <div 
          ref={emojiPickerRef}
          className="absolute bottom-16 left-3 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-3 z-50 w-80 max-h-48 overflow-y-auto"
        >
      ...
      <form className="flex items-center justify-between" onSubmit={handleSubmit}>
        <div className="relative w-full">
          <button
            type="button"
            className="absolute text-gray-500 -translate-y-1/2 left-1 top-1/2 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90 sm:left-3 z-10"
        ...
        <div className="flex items-center">
          ...
          <button
            type="button"
            className="mr-2 text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90 hidden"
          ...
          */}
          ...
          {/* Botón de documento - OCULTO
          <button
            type="button"
            className="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90 hidden"
            disabled={connectionStatus !== 'connected'}
          >
            <svg
          ...
          */}
          ...
          <button
            ...
            disabled={!message.trim() || isSubmitting || connectionStatus !== 'connected'}
          >
            {isSubmitting ? (
