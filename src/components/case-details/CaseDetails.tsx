...
import { Trash2, Loader2, FileDown, Pencil } from "lucide-react"
...
import { stageDefaultMessages, stageCases, STUDIO_NAME, STUDIO_PHONE } from "@/lib/constant"
...
export const CaseDetails = ({ caseData, onEdit, onDelete, onCaseUpdated }: CaseDetailsProps) => {
    ...
    const defaultMessage = fillMessageVariables(stageDefaultMessages[Number(caseData.stageId) || 1] || "")
    const [whatsappMessage, setWhatsappMessage] = useState(defaultMessage)
    const messageInputRef = useRef<HTMLTextAreaElement>(null)

    const currentStageLabel = stageCases.find((s) => s.value === Number(caseData.stageId))?.label || "Documentación"
    ...
    const handleLawyerChange = async (field: "responsibleLawyerId" | "internalLawyerId", lawyerId: number) => {
        try {
            const response = await fetch(`${CASES_ENDPOINT}/${caseData.id}`, {
                method: "PUT",
                headers: {
            ...
            toast.success(field === "responsibleLawyerId" ? "Abogado responsable actualizado" : "Abogado interno actualizado")
            onCaseUpdated?.()
        } catch (error) {
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        ...
                        <div className="min-w-0">
                            <div className="flex items-center gap-3">
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-semibold bg-gray-100 text-gray-600 shrink-0">
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <LawyerSelectDropdown
                                ...
                                type="responsible"
                                onLawyerChange={(id) => handleLawyerChange("responsibleLawyerId", id)}
                            />
                        </div>
                        ...
                        {/* Abogado Interno */}
                        <div className="flex items-center gap-2">
                            <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">Interno</span>
                            <LawyerSelectDropdown
                                ...
                                type="internal"
                                onLawyerChange={(id) => handleLawyerChange("internalLawyerId", id)}
                            />
                        </div>
                ...
                <div className="p-6">
                    ...
                    <div className="mb-5">
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                            Caso #{caseData.number ?? caseData.id} &middot; Etapa: <span className="font-medium text-[#09a4b5]">{currentStageLabel}</span>
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3 mb-4 p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate">{caseData.customer?.name || "Cliente"}</p>
                    ...
                    <textarea
                        ...
                        rows={6}
                        className="w-full rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 placeholder:text-gray-400 focus:border-green-300 focus:outline-none focus:ring-2 focus:ring-green-100 transition-colors resize-none leading-relaxed"
                    />
                    <p className="text-xs text-gray-400 mt-1.5">
                        El mensaje se genera automáticamente según la etapa. Podés editarlo antes de enviar.
                    </p>
