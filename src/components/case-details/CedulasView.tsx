...
import { FileText, Plus, CheckCircle2, Clock, XCircle, Pencil, Trash2, Send, User, Loader2, ChevronDown, Search, Printer, Download } from "lucide-react"
import { useSession } from "next-auth/react"
import { toast } from "sonner"
import { Editor } from "@tinymce/tinymce-react"
import { CASE_CEDULAS_ENDPOINT, CASE_CEDULA_BY_ID_ENDPOINT, CASES_FILES_BY_CASE_ID_ENDPOINT, CASE_PARTS_ENDPOINT } from "@/constant/api-endpoints"
...
const estadoConfig: Record<string, { label: string; color: string; icon: typeof CheckCircle2 }> = {
    generada: { label: "Generada", color: "bg-green-50 text-green-700 border-green-200 dark:bg-green-900/20 dark:text-green-300 dark:border-green-800", icon: CheckCircle2 },
...
export const CedulasView = ({ caseId, files = [], customerName }: CedulasViewProps) => {
    ...
    const fetchFileData = useCallback(async (fileId: number) => {
        ...
        try {
            ...
            const [fileRes, partsRes] = await Promise.all([
                fetch(CASES_FILES_BY_CASE_ID_ENDPOINT(Number(caseId), fileId), { headers }),
            ...
            if (partsRes.ok) {
                ...
                const parts = (partsData.parts || partsData || []).map((p: any) => ({ ...p, _source: "case" }))
                setFileParts(parts)
            } else {
                setFileParts([])
            }
    ...
    const personalizeTemplate = useCallback((template: string, customJuez?: string, customSecretaria?: string) => {
        ...
        const secretariaValue = customSecretaria || secretary || "[COMPLETAR DATO]"

        const variables: Record<string, string> = {
            "{{JUZGADO}}": charter && courtName ? `${charter} ${courtName}` : "[COMPLETAR JUZGADO]",
            "{{JURISDICCION}}": ciudad ? ciudad.toUpperCase() : "[COMPLETAR JURISDICCION]",
            "{{CIUDAD}}": ciudad || "[COMPLETAR CIUDAD]",
            "{{PROVINCIA}}": provincia || "[COMPLETAR PROVINCIA]",
            "{{CIUDAD_PROVINCIA}}": ciudad && provincia ? `${ciudad}. ${provincia}.` : "[COMPLETAR CIUDAD/PROVINCIA]",
            "{{FECHA}}": fecha,
            "{{JUEZ}}": juezValue,
    ...
    useEffect(() => {
        if (fileData && isCreating) {
            const baseTemplate = CEDULA_TEMPLATES[newCedula.cedulaType as keyof typeof CEDULA_TEMPLATES] || ""
            setNewCedula(prev => ({
                ...prev,
    ...
    const handleCedulaTypeChange = (type: string) => {
        const baseTemplate = CEDULA_TEMPLATES[type as keyof typeof CEDULA_TEMPLATES] || ""
        let updatedContent = personalizeTemplate(baseTemplate, newCedula.juez, newCedula.secretaria)
        if (newCedula.partId) {
            const selectedPart = fileParts.find(p => p.id.toString() === newCedula.partId)
            if (selectedPart) updatedContent = applyPartDataToContent(updatedContent, selectedPart)
        }
    ...
    const handleJuezChange = (juez: string) => {
        const baseTemplate = CEDULA_TEMPLATES[newCedula.cedulaType as keyof typeof CEDULA_TEMPLATES] || ""
        let updatedContent = personalizeTemplate(baseTemplate, juez, newCedula.secretaria)
        if (newCedula.partId) {
            const selectedPart = fileParts.find(p => p.id.toString() === newCedula.partId)
            if (selectedPart) updatedContent = applyPartDataToContent(updatedContent, selectedPart)
        }
    ...
    const handleSecretariaChange = (secretaria: string) => {
        const baseTemplate = CEDULA_TEMPLATES[newCedula.cedulaType as keyof typeof CEDULA_TEMPLATES] || ""
        let updatedContent = personalizeTemplate(baseTemplate, newCedula.juez, secretaria)
        if (newCedula.partId) {
            const selectedPart = fileParts.find(p => p.id.toString() === newCedula.partId)
            if (selectedPart) updatedContent = applyPartDataToContent(updatedContent, selectedPart)
        }
    ...
    const applyPartDataToContent = (content: string, selectedPart: any): string => {
        let updatedContent = content
        const partName = selectedPart.name ? selectedPart.name.toUpperCase() : ""
        const partAddress = selectedPart.address && selectedPart.address !== "0"
            ? `DOMICILIO: ${selectedPart.address.toUpperCase()}`
            : "DOMICILIO: [COMPLETAR DATO]"
        const city = selectedPart.city && selectedPart.city !== "0" ? selectedPart.city.toUpperCase() : ""

        const stateNames: Record<number, string> = {
            21: "SANTA FE", 1: "BUENOS AIRES", 2: "CATAMARCA", 3: "CHACO", 4: "CHUBUT",
            ...
            20: "SANTIAGO DEL ESTERO", 22: "TIERRA DEL FUEGO", 23: "TUCUMAN",
        }
        const province = selectedPart.stateId ? (stateNames[selectedPart.stateId] || "") : ""
        ...
        const regex1 = /(<p style="text-align: justify;[^"]*">[^<]*<span[^>]*><strong>)([^<]+)(<\/strong><\/span><\/p>\s*<p style="text-align: justify;[^"]*">[^<]*<span[^>]*><strong>)(DOMICILIO:[^<]+)(<\/strong><\/span>[^<]*<\/p>\s*<p style="text-align: justify;[^"]*">[^<]*<span[^>]*><strong>)([^<]+)(<\/strong>)/i
        if (regex1.test(updatedContent)) {
            updatedContent = updatedContent.replace(regex1, `$1${partName}$3${partAddress}$5${locationLine}$7`)
        }

        // Segundo bloque (si hay copia)
        const regex2 = /(<p style="text-align: justify;[^"]*">[^<]*<span[^>]*><strong>)([^<]+)(<\/strong><\/span><\/p>\s*<p style="text-align: justify;[^"]*">[^<]*<span[^>]*><strong>)(DOMICILIO:[^<]+)(<\/strong><\/span><\/p>\s*<p style="text-align: justify;[^"]*">[^<]*<span[^>]*><strong>)([^<]+)(<\/strong>)/i
        if (regex2.test(updatedContent)) {
            updatedContent = updatedContent.replace(regex2, `$1${partName}$3${partAddress}$5${locationLine}$7`)
        }
    ...
    const generadas = cedulas.filter(c => c.status === "generada" || c.status === "diligenciada").length
            ...
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
                ...
                {cedulas.length === 0 && !isCreating ? (
                    ...
                    <div className="p-4 space-y-3">
                        {cedulas.map((cedula) => {
                            const config = estadoConfig[cedula.status] || estadoConfig.generada
                            const Icon = config.icon
                            const cardClass = cedula.status === "diligenciada"
                                    ...
                                    <div className="flex items-start justify-between gap-4">
                                                    ...
                                                    {cedula.caseFile && (
                                                        <>
                                                            ...
                                                            <div className="flex items-center gap-1.5">
                                                                ...
                                                                <span>{getFileLabel(cedula.caseFile, customerName)}</span>
                                                            </div>
                                                        </>
                                                    )}
                                                    <span>•</span>
                                                    <span>{formatDate(cedula.generatedDate || cedula.createdAt)}</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-1.5 shrink-0">
                                            <button
                                                ...
                                                className="p-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-green-500 transition-colors"
                                            >
                                            ...
                                            <button
                                                ...
                                                className="p-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-blue-500 transition-colors"
                                            >
                ...
                <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
                    <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">Nueva Cédula</h2>
                                <p className="text-xs text-gray-500">Seleccioná un expediente y completá los datos</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-5 space-y-5">
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <div ref={fileRef} className="relative">
                                    ...
                                    >
                                        <span className={`truncate ${selectedFileId ? "text-gray-900 dark:text-gray-100" : "text-gray-400"}`}>{selectedFileLabel}</span>
                                        ...
                                        <div className="absolute z-50 mt-1 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg max-h-44 overflow-auto">
                                            ...
                                            {searchedFiles.map((f) => (
                                                <button
                                                    ...
                                                    className={`w-full text-left px-2.5 py-1.5 text-xs hover:bg-gray-50 dark:hover:bg-gray-700 truncate ${selectedFileId === f.id ? "bg-brand-500/10 text-brand-500" : "text-gray-700 dark:text-gray-200"}`}
                                                >
                                                    {getFileLabel(f, customerName)}
                                                </button>
                                            ))}
                                            {searchedFiles.length === 0 && (
                                                <div className="px-2.5 py-2 text-xs text-gray-400">No se encontraron expedientes</div>
                                            )}
                                        </div>
                                    )}
                                </div>
                        ...
                        {selectedFileId && (
                            <>
                                ...
                                <div className="space-y-3">
                                    <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Datos de la cédula</h3>
                                                ...
                                                <>
                                                    ...
                                                    >
                                                        ...
                                                        {fileParts.map((part: any) => (
                                                            <option key={`${part._source || "part"}-${part.id}`} value={part.id}>
                                                                {part.name} ({part.partyType}){part.documentNumber ? ` — ${part.documentType}: ${part.documentNumber}` : ""}
                                                            </option>
                                                        ))}
                                                    </select>
                                                    {fileParts.length === 0 && (
                                                        <p className="mt-1 text-xs text-amber-500">No hay partes vinculadas a este expediente. Agregá partes desde la pestaña Partes.</p>
                                                    )}
                                                </>
                                            )}
                                        </div>
                                ...
                                <div className="space-y-2">
                                    <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700 dark:text-gray-300">
                                        <FileText className="h-3.5 w-3.5 text-gray-400" />
                                        Contenido de la Cédula <span className="text-red-500">*</span>
                                    </label>
