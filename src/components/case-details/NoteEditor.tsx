        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg overflow-hidden py-1 min-w-[200px]">
            {items.map((item, index) => (
                <button
                    ...
                    className={`flex items-center gap-2 w-full px-3 py-2 text-left text-sm transition-colors ${index === selectedIndex
                        ? "bg-brand-50 dark:bg-brand-900/20 text-brand-700 dark:text-brand-300"
                        : "text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
                        }`}
                >
                    <div className="w-6 h-6 rounded-full bg-brand-100 dark:bg-brand-900/30 flex items-center justify-center text-xs font-semibold text-brand-600 shrink-0">
...
// ── Main editor ──
export const NoteEditor = ({ content, onChange, onMentionsChange, mentionUsers, placeholder = "Escribe tu comentario... Usa @ para mencionar" }: NoteEditorProps) => {
    ...
    const editor = useEditor({
        extensions: [
            ...
            Mention.configure({
                HTMLAttributes: {
                    class: "mention bg-brand-50 dark:bg-brand-900/20 text-brand-600 dark:text-brand-400 rounded px-1 py-0.5 font-medium text-sm",
                },
                suggestion: {
                    items: ({ query }: { query: string }) => {
                        return mentionUsersRef.current.filter((user) =>
        ...
        editorProps: {
            attributes: {
                class: "prose prose-sm max-w-none dark:prose-invert focus:outline-none min-h-[80px] px-4 py-3 text-sm",
            },
        },
    })
    ...
    const ToolbarBtn = ({ onClick, active, children, disabled }: { onClick: () => void; active?: boolean; children: React.ReactNode; disabled?: boolean }) => (
        <button
            type="button"
            onClick={onClick}
            disabled={disabled}
