...
import { NOTIFICATIONS_ENDPOINT, NOTIFICATIONS_READ_ENDPOINT, USERS_ENDPOINT } from "@/constant/api-endpoints"
...
export function NotificationProvider({ children }: { children: ReactNode }) {
    ...
    const refreshNotifications = useCallback(async () => {
        ...
        if (!userId) {
            console.log("No currentUserId, skipping refresh. CurrentUserId:", currentUserId, "Ref:", currentUserIdRef.current)
            return
        }
        ...
        try {
            ...
            if (!Array.isArray(notificationsArray)) {
                ...
                const formattedNotifications = notificationsArray.map((notification: any) => ({
                    id: notification.id,
        ...
        try {
            ...
            setNotifications((prev) =>
                prev.map((notification) =>
                    notification.id.toString() === id.toString() ? { ...notification, read: true } : notification,
                ),
            )
    ...
    useEffect(() => {
        if (session?.user?.id && !currentUserIdRef.current) {
            console.log("Setting currentUserId from session:", session.user.id)
    ...
    useEffect(() => {
        ...
        if (!currentUserIdRef.current && session?.user) {
            ...
            if (userId) {
                console.log("Setting currentUserId from session:", userId)
